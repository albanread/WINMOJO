...
PATCHES = [
    ...
    # The lldb overlay's liblldb arrangement (linkshared cc_binary + cc_import
    # wrapper) collides with itself on Windows: the extensionless target name
    # makes Bazel synthesize the .dll artifact at the same path cc_import
    # materializes ("generated by these conflicting actions"), and cc_import
    # has no interface library to link against. This patch adds a Windows
    # spelling with an explicit .dll name and the interface_library output
    # group wired through.
    "//bazel/public-patches:llvm-lldb-windows-dll.patch",
    # This tree links clang statically; without CLANG_BUILD_STATIC the
    # CLANG_ABI annotations expand to dllimport on Windows and the defining
    # TUs drop their bodies. See the comment in the patch.
    "//bazel/public-patches:llvm-clang-static-abi.patch",
    # The Windows process plugin's srcs glob was non-recursive and missed the
    # per-architecture register contexts in subdirectories.
    "//bazel/public-patches:llvm-lldb-windows-arm64-regctx.patch",
]
